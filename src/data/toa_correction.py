...
def toa_correct_dataset_KTH(params):
    print('IN TOA CORRECT')
    data_path = params.project_path + "data/raw/KTH_TOA_RAW/"
    data_toa_path = params.project_path + "data/processed/KTH_TOA/"
    data_output_path = params.project_path + "data/output/KTH_TOA_CORRECT/"
    ...
    for product in products:
        print(product)
        p2 = data_path[:-1] + '/' + product
        p1 = 'gdal_translate'
        # Save individual bands temporarily

        for b in range(1, 4):
            p3 = data_output_path + product[:-4] + 'B' + str(b) + '.tif'
...
def toa_correct_dataset_SPARCS(params):
    print('IN TOA CORRECT')
