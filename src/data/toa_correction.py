...
def toa_correct_dataset(params):
    ...
    data_output_path = params.project_path + "data/processed/SPARCS_TOA/"
    ...
    for product in products:
        ...
        p3 = data_output_path[:-1] + '/' + product[0:24] + '_toa.TIF'
        cmd = [p1, '-of', 'Gtiff', d4, p3]
        subprocess.run(cmd)
        print('IN TOA CORRECT 9')

        # Delete temp files
        subprocess.run('del *.tif *.img *.xml', shell=True, cwd=data_output_path)
